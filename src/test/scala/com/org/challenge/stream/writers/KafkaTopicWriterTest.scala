...
class KafkaTopicWriterTest extends AnyFunSuite with BeforeAndAfterAll {
  ...
  test("when serialization is json, final dataframe contains only key and value as column") {
    val appParams = new ParamsBuilder()
      ...
      .withKafaWriterSerialization(Some(KafkaSerialization.JsonSerialization.toString))
      .build()
  ...
  test("when serialization is avro, final dataframe contains only key and value as column") {
    val appParams = new ParamsBuilder()
      .withReaderType(ReaderFactory.KafkaReader.toString)
      .withTopics(Some(Seq("pageviews", "users")))
      .withDelayPerTopic("users", 20L)
      ...
      .withSchemaManager("dummy")//required for input stream scaffolding, not for transformation itself
      .withKafaInputSerialization(Some(KafkaSerialization.JsonSerialization.toString))
      .withKafaWriterSerialization(Some(KafkaSerialization.AvroSerialization.toString))
      .build()
    ...
    Mockito.doAnswer(new Answer[DataStreamWriter[Row]] {
      override def answer(invocation: InvocationOnMock): DataStreamWriter[Row] = {
        invocation.getArgument[Option[DataFrame]](0)
          ...
          .foreachBatch((batchDf: DataFrame, batchId: Long) => {
            kafkaSpy.getFinalDF(transformationSpy.transformBatch(Some(batchDf)).get)
              .createOrReplaceGlobalTempView("kafka_ready2")
          })
      }
    }).when(kafkaSpy).writer(transformedDF, transformation)
    ...
    this.sparkSession.sql("SELECT * FROM global_temp.kafka_ready2").show(20, false)
    Thread.sleep(10000L)
    assert(this.sparkSession.sql("SELECT * FROM global_temp.kafka_ready2").schema.fields.map(_.name).length == 2)
    assert(this.sparkSession.sql("SELECT * FROM global_temp.kafka_ready2").count() == 4)
  }
